...
final class MenuCalendarInteractionTests: XCTestCase {
    ...
    func testHoverCrossingPinAndClickSuppression() throws {
        ...
        card.updatePointer(.zero, now: now.addingTimeInterval(6))
        card.updatePointer(.zero, now: now.addingTimeInterval(7))
        XCTAssertFalse(panel.isVisible)
    }
    @MainActor
