...
快捷键统一为 Option＋字母/数字，自动迁移旧工具绑定并保留已占用键位；键位显示目标名称，点击弹出绑定类型菜单，再选择应用或 OneBoard 工具。菜单栏 OneBoard、日历和网速使用同高单色图形，网速上下排列；本机运行超过 24 小时显示天数和小时。截图工具栏增加系统取色器，样式参数按需展开。
...
- 长截图统一首帧与后续 ScreenCaptureKit 采集路径，支持低纹理文本匹配；选区外暗色遮罩、右侧实时预览和紧凑完成/取消栏。不支持没有重叠的跳页及任意动态内容。
