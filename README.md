...
刘海悬停连续 0.5 秒展开，离开刘海与面板后收起；拖放同时覆盖 HostingView 的 AppKit 回调。色板保持固定位置，数值控件分别表示字号、线宽、编号和马赛克颗粒。文字单击选中后可拖动，双击复用首次输入的原位编辑器。

## 2026-09-07 暂存拖放与标注视觉

刘海暂存修复原生拖入/拖出命中，文件通过系统 URL 中转；展开收起仅缩放、不淡入淡出。文字输入取消材质底和整圈边框，马赛克使用不透明固定方格。
...
供应商预设与额度自动/手动配置、直接绑定选择器、授权页目录书签，以及截图字号调节、框选箭头文字标注、实时 RGB 取色（⌘C 复制）已更新。长截图仅保留四角提示，不再用四块暗色遮罩形成大框。
