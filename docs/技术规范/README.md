...
FileShelfHostingView 注册系统文件类型并显式转交 draggingEntered/Updated/prepare/perform，空白区域回退为暂存。悬停状态按注入时间测试，Timer 加入 common RunLoop；不再用任意鼠标按键按下阻止关闭。已有文字使用 isTextInput 与编辑图层 ID 复用编辑器，提交保留原图层 ID，文字编辑及一次拖动各记录一次撤销。

## 2026-09-07 暂存拖放与标注视觉

FileShelfHostingView 将拖放区域的 hitTest 转交原生 Destination/Source；拖出以 NSURL 创建 NSDraggingItem。NSPanel 保持 alphaValue=1，SwiftUI 按窗口尺寸从顶部缩放。MosaicPixels 按原图坐标对齐方格后裁剪，预览与导出共享像素。
...
CalendarMonthScrollView 仅在所属窗口和日期区内消费 scrollWheel，按方向生成月份偏移，忽略触控板惯性并按手势去重。ScreenshotCaptureService 向每屏遮罩传入本屏 visibleFrame 的局部坐标；工具栏优先选区下方、其次上方，均不足则落在可用区底部上方 12pt。
